...
const cleanDir = (dir) => {
    for (const file of fs.readdirSync(dir, { withFileTypes: true })) {
        if (file.name === 'mt.parser.js' || file.name === 'mt.grammar.terms.js') {
...
const processFile = async (file) => {
    if (file) {
        ...
        try {
            ...
            //fs.writeFileSync(path.resolve(srcDir, 'mt.parser.js'), parser);
            fs.writeFileSync(path.resolve(srcDir, 'mt.grammar.terms.js'), terms);
        } catch (e) {
...
chokidar
    .watch([path.resolve(srcDir, 'mt.grammar')], {
